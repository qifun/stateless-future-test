...
import com.qifun.immutableFuture.Future.ToConcurrentFuture
...
class FutureSpec {
  ...
  def testAsync(s: String)(implicit ec: ExecutionContext): Future[String] = s match {
    ...
    case "Failure" => future { throw new RuntimeException("Expected exception; to test fault-tolerance") }
  ...
  @Test def `recoverWith from exceptions`() {
    ...
    intercept[IllegalStateException] {
      val failed = future[String] { throw o } recoverWith {
        case _ if false == true => future("yay!")
      }
      Await.result(failed, defaultTimeout)
    } mustBe (o)

    val recovered = future[String] { throw o } recoverWith {
      case _ => future("yay!")
    }
    Await.result(recovered, defaultTimeout) mustBe ("yay!")

    intercept[IllegalStateException] {
      val refailed = future[String] { throw o } recoverWith {
        case _ => future[String] { throw r }
      }
  ...
  @Test def `find`() {
    ...
    val result = scala.concurrent.Future.find[Int](futures)(_ == 3)
    Await.result(result, defaultTimeout) mustBe (Some(3))

    val notFound = scala.concurrent.Future.find[Int](futures.iterator)(_ == 11)
  ...
  @Test def `zip`() {
    ...
    intercept[IllegalStateException] {
      val failed = future[String] { throw f } zip future("foo")
      Await.result(failed, timeout)
    } mustBe (f)

    intercept[IllegalStateException] {
      val failed = future("foo") zip future[String] { throw f }
      Await.result(failed, timeout)
    } mustBe (f)

    intercept[IllegalStateException] {
      val failed = future[String] { throw f } zip future[String] { throw f }
      Await.result(failed, timeout)
    } mustBe (f)

    val successful = future("foo") zip future("foo")
  ...
  @Test def `fold`() {
    ...
    val foldedit = scala.concurrent.Future.fold(futures)(0)(_ + _)
  ...
  @Test def `fold with an exception`() {
    val timeout = 10000 millis
    def async(add: Int, wait1: Int) = future {
      Thread.sleep(wait1)
  ...
  @Test def `return zero value if folding empty list`() {
    val zero = scala.concurrent.Future.fold(List[scala.concurrent.Future[Int]]())(0)(_ + _)
  ...
  @Test def `shouldReduceResults`() {
    ...
    val reduced = scala.concurrent.Future.reduce(futures)(_ + _)
    Await.result(reduced, timeout) mustBe (45)

    val futuresit = (0 to 9) map { async }
    val reducedit = scala.concurrent.Future.reduce(futuresit)(_ + _)
  ...
  @Test def `shouldReduceThrowNSEEOnEmptyInput`() {
    intercept[java.util.NoSuchElementException] {
      val emptyreduced = scala.concurrent.Future.reduce(List[scala.concurrent.Future[Int]]())(_ + _)
  ...
  @Test def `shouldTraverseFutures`() {
    ...
    val traversedList = scala.concurrent.Future.traverse(list)(x => future(x * 2 - 1))
    Await.result(traversedList, defaultTimeout).sum mustBe (10000)

    val iterator = (1 to 100).toList.iterator
    val traversedIterator = scala.concurrent.Future.traverse(iterator)(x => future(x * 2 - 1))
