...
class NakedAwait {
  ...
  def `await only allowed in async neg`() {
    import _root_.com.qifun.immutableFuture.test.Async._
    expectError("`await` must be enclosed in a `Future` block") {
      """
        | import _root_.com.qifun.immutableFuture.test.Async._
        | (null: _root_.com.qifun.immutableFuture.Future[Any]).await
      """.stripMargin
    }
  }
  ...
  def `await not allowed in by-name argument`() {
    expectError("await must not be used under a by-name argument.") {
      """
        | import _root_.com.qifun.immutableFuture.test.internal.AsyncId._
