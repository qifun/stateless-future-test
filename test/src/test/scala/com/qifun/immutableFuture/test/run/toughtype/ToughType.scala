...
class ToughTypeSpec {
  ...
  @Test def `propogates tough types`() {
    ...
    val res: (List[_], com.qifun.immutableFuture.test.run.toughtype.ToughTypeObject.Inner) = Await.result(fut, 2 seconds)
    res._1 mustBe (Nil)
  }
  ...
  @Test def existentialBind2Issue19() {
    import com.qifun.immutableFuture.test.Async._, scala.concurrent.ExecutionContext.Implicits.global
